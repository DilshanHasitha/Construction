...
type RestOf<T extends IOrders | NewOrders> = Omit<T, 'orderPlacedOn'> & {
...
export class OrdersService {
  ...
  protected convertDateFromClient<T extends IOrders | NewOrders | PartialUpdateOrders>(orders: T): RestOf<T> {
    return {
      ...orders,
      orderPlacedOn: orders.orderPlacedOn?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrders: RestOrders): IOrders {
    return {
      ...restOrders,
      orderPlacedOn: restOrders.orderPlacedOn ? dayjs(restOrders.orderPlacedOn) : undefined,
    };
  }
