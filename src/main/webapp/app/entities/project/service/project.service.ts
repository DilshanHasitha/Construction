...
type RestOf<T extends IProject | NewProject> = Omit<T, 'completionDate'> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      completionDate: project.completionDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      completionDate: restProject.completionDate ? dayjs(restProject.completionDate) : undefined,
    };
  }
